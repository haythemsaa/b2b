...
<div class="container" x-data="cartManager()" x-init="init()">
    <div class="row mb-4 animate__animated animate__fadeInDown">
        <div class="col-12 d-flex justify-content-between align-items-center">
            <div>
                <h1><i class="bi bi-cart3 me-2 text-primary"></i>{{ __('Shopping Cart') }}</h1>
                <p class="text-muted mb-0">
                    <span x-show="items.length > 0">
                        <span x-text="items.length"></span> {{ __('item(s)') }} -
                        <span x-text="totalItems"></span> {{ __('product(s)') }}
                    </span>
                </p>
            </div>
    ...
    <div x-show="items.length === 0" class="row">
        <div class="col-12 text-center py-5 animate__animated animate__fadeIn">
            ...
            <h4 class="text-muted mb-3">{{ __('Your cart is empty') }}</h4>
            <p class="text-muted mb-4">{{ __('Add some products to your cart to continue shopping') }}</p>
        ...
        <div class="col-lg-8 mb-4">
            <div class="card shadow-sm animate__animated animate__fadeInLeft">
                <div class="card-header bg-primary text-white">
                    <h5 class="mb-0"><i class="bi bi-bag me-2"></i>{{ __('Cart Items') }}</h5>
                </div>
                <div class="card-body">
        ...
        <div class="col-lg-4">
            <div class="card shadow-sm position-sticky animate__animated animate__fadeInRight" style="top: 20px;">
                <div class="card-header bg-success text-white">
                    <h5 class="mb-0"><i class="bi bi-receipt me-2"></i>{{ __('Order Summary') }}</h5>
                </div>
                <div class="card-body">
                    <!-- Subtotal -->
                    ...
                    <!-- Tax -->
                    <div class="d-flex justify-content-between mb-2 pb-2">
                        <span>{{ __('Tax') }} (<span x-text="taxRate + '%'"></span>):</span>
                    ...
                    <div x-show="totalDiscount > 0" class="alert alert-success mb-3">
                        ...
                        {{ __('You save') }}: <strong x-text="formatPrice(totalDiscount)"></strong>
                    </div>
                    ...
                    <div class="mt-4 pt-3 border-top">
                        <div class="d-flex align-items-center mb-2">
                            ...
                            <small class="text-muted">{{ __('Secure Payment') }}</small>
                        </div>
                        <div class="d-flex align-items-center mb-2">
                            <i class="bi bi-truck text-success me-2"></i>
                            <small class="text-muted">{{ __('Fast Delivery') }}</small>
                        </div>
                        <div class="d-flex align-items-center">
                            <i class="bi bi-arrow-return-left text-success me-2"></i>
                            <small class="text-muted">{{ __('Easy Returns') }}</small>
                        </div>
                    </div>
                </div>
            </div>
    ...
    return {
        ...
        async removeItem(productId) {
            const result = await Swal.fire({
                ...
                text: '{{ __("Are you sure you want to remove this item from cart?") }}',
                icon: 'warning',
        ...
        async clearCart() {
            const result = await Swal.fire({
                title: '{{ __("Clear cart?") }}',
                text: '{{ __("This will remove all items from your cart") }}',
                icon: 'warning',
                showCancelButton: true,
            ...
            try {
                ...
                if (data.success) {
                    Swal.fire({
                        ...
                        icon: 'success',
                        confirmButtonText: '{{ __("View Orders") }}'
                    }).then(() => {
