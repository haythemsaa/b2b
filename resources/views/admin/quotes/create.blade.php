...
<div class="container-fluid">
    ...
    <form action="{{ route('admin.quotes.store') }}" method="POST" id="quote-form">
        @csrf
                            ...
                            <div class="col-md-6 mb-3">
                                ...
                                <select class="form-select @error('user_id') is-invalid @enderror"
                                    ...
                                    @foreach($vendeurs as $vendeur)
                                        <option value="{{ $vendeur->id }}" {{ old('user_id') == $vendeur->id ? 'selected' : '' }}>
                                            {{ $vendeur->name }} ({{ $vendeur->email }})
                                        </option>
                                    @endforeach
                                </select>
                                @error('user_id')
                    ...
                    <div class="card-body">
                        ...
                        <div class="mb-3">
                            ...
                            <textarea class="form-control @error('terms') is-invalid @enderror"
                                      id="terms"
                                      name="terms"
                                      rows="4">{{ old('terms', 'Paiement sous 30 jours. Marchandise non reprise.') }}</textarea>
                            @error('terms')
                                <div class="invalid-feedback">{{ $message }}</div>
                            @enderror
                        </div>
            ...
            <div class="col-lg-4">
                <div class="card mb-4 sticky-top" style="top: 20px;">
                    ...
                    <div class="card-body">
                        ...
                        <a href="{{ route('admin.quotes.index') }}" class="btn btn-secondary w-100">
                    ...
                    <select class="form-select item-product" name="items[INDEX][product_id]" required onchange="updatePrice(this)">
                        ...
                        @foreach($products as $product)
                            <option value="{{ $product->id }}" data-price="{{ $product->price }}">
                                {{ $product->name }} ({{ number_format($product->price, 2) }} TND)
                            </option>
                        @endforeach
                    </select>
                </div>
